 ...
 * @returns 增强后的元数据
 */
export async function enrichMetadataWithAI(metadata: Metadata, options: MetadataEnrichOptions): Promise<Metadata> {
 ...
 * @returns 增强后的仓库列表
 */
export async function enrichWithAI(repositories: Repository[], options: MetadataEnrichOptions): Promise<Repository[]> {
  const config = getConfig();
  ...
  const enrichPromises = repositories.map(async (repo) => {
    try {
      ...
      // 如果 AI 增强失败，返回原始数据
      console.error(`对仓库 ${repo.author}/${repo.name} 的 AI 增强失败: ${(error as Error).message}`);
      return repo;
    }
  });
